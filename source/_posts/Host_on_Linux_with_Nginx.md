...
这篇讲如何在 Ubuntu 上部署 .NET Core 站点<!--more-->

{% asset_img profit.png %}
...
# 设置反向代理
反向代理将终止 HTTP 请求并将其转发到 ASP.NET 核心应用程序
反向代理服务器可以减轻诸如服务静态内容、缓存请求、压缩请求和 HTTP 服务器的 SSL 终止等工作
...
```
> 只有若有更改则需运行`systemctl daemon-reload`来重新载入配置
